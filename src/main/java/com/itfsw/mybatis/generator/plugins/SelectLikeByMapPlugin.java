...
public class SelectLikeByMapPlugin extends BasePlugin {

    private static final String SELECT_LIKE_MAP_METHOD_NAME = "selectLikeByMap";

    @Override
    public boolean clientGenerated(Interface interfaze, TopLevelClass topLevelClass, IntrospectedTable introspectedTable) {
        Method selectLikeModelMethod = JavaElementGeneratorTools.generateMethod(
                SELECT_LIKE_MAP_METHOD_NAME,
                JavaVisibility.DEFAULT,
                new FullyQualifiedJavaType("List<" + introspectedTable.getBaseRecordType() + ">"),
                new Parameter(new FullyQualifiedJavaType("Map<String, String>"), "map")
        );
    ...
    public boolean sqlMapDocumentGenerated(Document document, IntrospectedTable introspectedTable) {
        ...
        selectLikeModel.addAttribute(new Attribute("id", SELECT_LIKE_MAP_METHOD_NAME));
        selectLikeModel.addAttribute(new Attribute("parameterType", FullyQualifiedJavaType.getNewHashMapInstance().getFullyQualifiedName()));
        ...
        columnList.forEach(column -> {
            ...
            check.addElement(new TextElement(String.format(
                    "and %s like #{_parameter[%s], jdbcType=%s}",
